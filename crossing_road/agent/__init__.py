...
class GeneratePDDL_Stationary :
    ...
    def generateGridCells(self) :
        ...
        for t in range(self.width): #adding time as the third dimension in the gridcell
            for w in range(self.width) :
                for lane in range(self.num_lanes) :
                    self.grid_cell_list.append("pt{}pt{}pt{}".format(t, w, lane))
    ...
    def generateInitString(self) :
        ...
        '''  
        # agent's position
        # occupied = [self.state.occupancy_trails for x in range(self.width)]

        agent_pos_string = self.grid_cell_list[self.state.agent.position.x * self.num_lanes + self.state.agent.position.y]

        init_agent_pos_string = "(at " + agent_pos_string + " agent1" + ")" + "(blocked " + agent_pos_string +")"

        cars_pos_string = ""
        for car in self.state.cars:
            for t in range(self.width):
                if t == 0:
                    car_pos_string = "pt{}pt{}pt{}".format(t, abs((car.position.x + car.speed_range[0]*t) % self.width), car.position.y)
                    cars_pos_string += "(blocked " + car_pos_string + ")\n"
                else:
                    for s in range(abs(car.speed_range[0])):
                        car_pos_string = "pt{}pt{}pt{}".format(t, abs((car.position.x + car.speed_range[0]*t + s) % self.width), car.position.y)
                        cars_pos_string += "(blocked " + car_pos_string + ")\n"
            #     # occupied[t][abs((car.position.x + car.speed_range[0]*t) % self.width), car.position.y] = 1
            # import pdb
            # pdb.set_trace()


        # generate action initial state

        ## UP
        up_string = ""
        #for the boundary position
        for t in range(self.width - 1):
            for pt in range(1, self.width):
                up_string += "(up_next pt{}pt{}pt0 pt{}pt{}pt0)".format(t, pt, t + 1, pt - 1)
        
        for pt in range(1, self.width): # when t = width -1
            up_string += "(up_next pt{}pt{}pt0 pt{}pt{}pt0)".format(self.width - 1, pt, 0, pt - 1)

        #for the non-boundary position
        for t in range(self.width - 1):
            for pty in range(1, self.num_lanes):
                for ptx in range(1, self.width):
                    up_string += "(up_next pt{}pt{}pt{} pt{}pt{}pt{})".format(t, ptx, pty, t + 1, ptx - 1, pty - 1)
        
        for pty in range(1, self.num_lanes): # when t = width -1
            for ptx in range(1, self.width):
                up_string += "(up_next pt{}pt{}pt{} pt{}pt{}pt{})".format(self.width - 1, ptx, pty, 0, ptx - 1, pty - 1)
        ...
        for t in range(self.width - 1):
            for pt in range(1, self.width):
                up_string += "(down_next pt{}pt{}pt{} pt{}pt{}pt{})".format(t, pt, self.num_lanes - 1, t + 1, pt - 1, self.num_lanes - 1)
        
        for pt in range(1, self.width): # when t = width -1
            up_string += "(up_next pt{}pt{}pt{} pt{}pt{}pt{})".format(self.width - 1, pt, self.num_lanes - 1, 0, pt - 1, self.num_lanes - 1)
        ...
        for t in range(self.width - 1):
            for pty in range(1, self.num_lanes):
                for ptx in range(1, self.width):
                    up_string += "(down_next pt{}pt{}pt{} pt{}pt{}pt{})".format(t, ptx, pty, t + 1, ptx - 1, pty + 1)
        
        for pty in range(1, self.num_lanes): # when t = width -1
            for ptx in range(1, self.width):
                up_string += "(down_next pt{}pt{}pt{} pt{}pt{}pt{})".format(self.width - 1, ptx, pty, 0, ptx - 1, pty + 1)


        ## FORWARD[-1]
        forward1_string = ""
        for t in range(self.width - 1):
            for pty in range(self.num_lanes):
                for ptx in range(1, self.width):
                    forward1_string += "(forward[-1]_next pt{}pt{}pt{} pt{}pt{}pt{})".format(t, ptx, pty, t + 1, ptx - 1, pty)
        
        for pty in range(self.num_lanes): # when t = width -1
            for ptx in range(1, self.width):
                forward1_string += "(forward[-1]_next pt{}pt{}pt{} pt{}pt{}pt{})".format(self.width - 1, ptx, pty, 0, ptx - 1, pty)
        ...
        for t in range(self.width - 1):
            for pty in range(self.num_lanes):
                for ptx in range(1, self.width):
                    forward2_string += "(forward[-2]_next pt{}pt{}pt{} pt{}pt{}pt{})".format(t, ptx, pty, t + 1, ptx - 2, pty)
        
        for pty in range(self.num_lanes): # when t = width -1
            for ptx in range(1, self.width):
                forward2_string += "(forward[-2]_next pt{}pt{}pt{} pt{}pt{}pt{})".format(self.width - 1, ptx, pty, 0, ptx - 2, pty)
        ...
        for t in range(self.width - 1):
            for pty in range(self.num_lanes):
                for ptx in range(1, self.width):
                    forward3_string += "(forward[-3]_next pt{}pt{}pt{} pt{}pt{}pt{})".format(t, ptx, pty, t + 1, ptx - 3, pty)
        
        for pty in range(self.num_lanes): # when t = width -1
            for ptx in range(1, self.width):
                forward3_string += "(forward[-3]_next pt{}pt{}pt{} pt{}pt{}pt{})".format(self.width - 1, ptx, pty, 0, ptx - 3, pty)

        init_string = init_agent_pos_string + cars_pos_string + up_string + down_string + forward1_string + forward2_string + forward3_string
        return init_string
    ...
    def generateGoalString(self) :
        ...
        agent1_goal_string = ""
        for t in range(self.width):
            agent1_goal_string += "(not (at pt{}pt{}pt{} agent1))".format(t, self.state.finish_position.x, self.state.finish_position.y)
        
        agent1_goal_string = "(not (and " + agent1_goal_string + "))"
        return agent1_goal_string
...
def generateDomainPDDLFile(gen):
    ...
    gen.addPredicate(name="forward[-1]_next", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")))
    gen.addPredicate(name="forward[-2]_next", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")))
    gen.addPredicate(name="forward[-3]_next", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")))
    ...
    gen.addAction(name="UP",
                  ...
                  precondition_string="(and (at ?from ?car1) (not (blocked ?to)) (up_next ?from ?to))",
                  effect_string="(and (not (blocked ?from)) (not (at ?from ?car1)) (at ?to ?car1) (blocked ?to))")

    gen.addAction(name="DOWN",
                  parameters=(("car1", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (at ?from ?car1) (not (blocked ?to)) (down_next ?from ?to))",
                  effect_string="(and (not (blocked ?from)) (not (at ?from ?car1)) (at ?to ?car1) (blocked ?to))")

    gen.addAction(name="FORWARD[-1]",
                  parameters=(("car1", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (at ?from ?car1) (not (blocked ?to)) (forward[-1]_next ?from ?to))",
                  effect_string="(and (not (blocked ?from)) (not (at ?from ?car1)) (at ?to ?car1) (blocked ?to))")

    gen.addAction(name="FORWARD[-2]",
                  parameters=(("car1", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (at ?from ?car1) (not (blocked ?to)) (forward[-2]_next ?from ?to))",
                  effect_string="(and (not (blocked ?from)) (not (at ?from ?car1)) (at ?to ?car1) (blocked ?to))")

    gen.addAction(name="FORWARD[-3]",
                  parameters=(("car1", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (at ?from ?car1) (not (blocked ?to)) (forward[-3]_next ?from ?to))",
                  effect_string="(and (not (blocked ?from)) (not (at ?from ?car1)) (at ?to ?car1) (blocked ?to))")
...
def simulateSolution(env):
    ...
    for line in plan_file.readlines() :
        if line[0] == '(' :
            action = line.split()[0][1:] #[0:up, 1:down, 2:forward[-3], 3:forward[-2], 4:forward[-1]]
            print(action)
            if action == 'up' :
