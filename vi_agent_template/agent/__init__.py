...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    _value_fn = value_fn.copy()

    while True:
        value_fn = _value_fn.copy()
        _delta = 0
        for s in range(len(value_fn)):
            possible_actions = matmul(trn_fn[:, s, :] , (rwd_fn[s, :] + gamma * _value_fn))
            _value_fn[s] = np.max(possible_actions)
            d = abs(_value_fn[s] - value_fn[s])
            if _delta < d:
                _delta = d
        if _delta < delta:
            break
...
if not SUBMISSION:
    ### Sample test cases. 
    test_config = [{'lanes' : [LaneSpec(0, [-2, -1])] *5,'width' :9, 'gamma' : 0.9, 'seed' : 15, 'fin_pos' : Point(1,1), 'agent_pos': Point(8,4),'stochasticity': 1.  },
