...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for car in self.state.cars:
            car_pos_string = "pt{}pt{}".format((car.position.x % self.width), car.position.y)
        ...
        for pt in range(1, self.width):
            up_string += "(up_next pt{}pt0 pt{}pt0)".format(pt, pt-1)

        #for the non-boundary position
        for pty in range(1, self.num_lanes):
            for ptx in range(1, self.width):
                up_string += "(up_next pt{}pt{} pt{}pt{})".format(ptx, pty, ptx - 1, pty - 1)
        ...
        # for the boundary position
        for pt in range(1, self.width):
            up_string += "(down_next pt{}pt{} pt{}pt{})".format(pt, self.num_lanes - 1, pt - 1, self.num_lanes - 1)

        # for the non-boundary position
        for pty in range(1, self.num_lanes):
            for ptx in range(1, self.width):
                up_string += "(down_next pt{}pt{} pt{}pt{})".format(ptx, pty, ptx - 1, pty + 1)
        ...
        for pty in range(self.num_lanes):
            for ptx in range(1, self.width):
                forward_string += "(forward_next pt{}pt{} pt{}pt{})".format(ptx, pty, ptx - 1, pty)

        init_string = init_agent_pos_string + cars_pos_string + up_string + down_string + forward_string
        return init_string
    ...
    def generateGoalString(self) :
        ...
        '''    
        agent1_goal_string = "(not (at pt{}pt{} agent1))".format(self.state.finish_position.x, self.state.finish_position.y)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="FORWARD",
                  parameters=(("car1", "agent"), ("from", "gridcell"), ("to", "gridcell")),
                  precondition_string="(and (at ?from ?car1) (not (blocked ?to)) (forward_next ?from ?to))",
                  effect_string="(and (not (blocked ?from)) (not (at ?from ?car1)) (at ?to ?car1) (blocked ?to))")
