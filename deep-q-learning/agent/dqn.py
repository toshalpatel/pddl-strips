...
class ReplayBuffer():
    ...
    def push(self, transition):
        ...
        if len(self.buffer) < self.buffer_limit:
            ...
            self.buffer.append(transition)
            self.buffer = self.buffer[len(self.buffer) - self.buffer_limit:]
        pass
    ...
    def sample(self, batch_size):
        ...
        '''
        torch.device(device)
        next_index = self.sample_index + batch_size
        if next_index < len(self.buffer):
            sample = self.buffer[self.sample_index:next_index]
        else:
            sample = self.buffer[self.sample_index:]
            next_index = next_index % len(self.buffer)
            sample = sample + self.buffer[:next_index]
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        Input:
            * `state` (`torch.tensor` [batch_size, channel, height, width]) 1*4*4*10
        ...
        optim_action = torch.argmax(action).tolist()
        is_greedy = (random.random() > epsilon)
        rand_action = random.randint(0, self.num_actions - 1)
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    q = model(states)
    q_a = torch.gather(q, 1, actions.long()).squeeze()
    q_t = torch.max(target(next_states) * (1 - dones), 1)[0]
    mseLoss = nn.MSELoss()
    huberLoss = nn.SmoothL1Loss()
    m_loss = mseLoss(q_a, rewards.squeeze() + gamma * q_t)
    # h_loss = huberLoss(q_a, rewards.squeeze() + gamma * q_t)
    return m_loss
